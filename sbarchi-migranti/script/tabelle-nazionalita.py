...
tables = tabula.read_pdf(args.input_file, pages=args.page,lattice=True, pandas_options={'dtype': str})

# Estrazione della prima tabella (se presente)
if len(tables) > 0:
    table = tables[0]
    output_file = os.path.splitext(args.input_file)[0] + '-nazionalita.csv'
